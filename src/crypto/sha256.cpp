...
std::array<uint8_t, 32> sha256_impl(const uint8_t* data, size_t len) {
    uint32_t H[8];
...
std::array<uint8_t, 32> double_sha256(const uint8_t* data, size_t len) {
    auto first = sha256_impl(data, len);
    return sha256_impl(first.data(), first.size());
}

std::array<uint8_t, 32> double_sha256(const std::vector<uint8_t>& data) {
    auto first = sha256_impl(data.data(), data.size());
    return sha256_impl(first.data(), first.size());
}
